...
> 2026-07-12: 안정화된 모바일 구름 애니메이션의 선명도를 높인다. 재생 안정성을 위해 12fps/60프레임은 유지하고 해상도를 640×360에서 768×432로, WebP 품질을 64에서 70으로 높인다. 최종 파일은 약 0.60MB로 모바일 디코딩 부담을 과도하게 늘리지 않는다.

> 2026-07-12: 모바일 animated WebP가 캐시의 기존 재생 타임라인을 공유해 중간 프레임부터 시작하는 문제를 수정한다. 자산을 Blob으로 한 번 메모리 캐시하고 매 전환마다 새 Object URL과 새 이미지 디코더를 생성하며, 첫 프레임 load 완료 후 overlay를 표시한다. 이전 Object URL은 즉시 revoke한다.

> 2026-07-12: 모바일 투명 구름 애니메이션의 디코딩 끊김을 줄인다. 자산을 960×540/15fps/76프레임/0.93MB에서 640×360/12fps/60프레임/0.44MB로 최적화해 RGBA 디코딩 픽셀량을 약 65% 줄이고, 모바일 preload·비동기 디코딩·GPU 합성 힌트를 적용한다.

> 2026-07-12: 기존 구름 소스의 검정 매트를 실제 알파로 키잉한 모바일 전용 `cloud-transition-mobile.webp`를 생성한다. 960×540, 15fps, 76프레임, 약 0.93MB의 투명 애니메이션이며 모바일은 새 이미지 요소를 복제해 매 전환마다 재생을 다시 시작한다. PC는 기존 WebM을 유지하고 화면 교체 타이밍은 양쪽 모두 동일하게 둔다.

> 2026-07-12: 모바일 구름 트랜지션을 PC와 완전히 동일하게 다시 활성화한다. 모바일 전용 즉시 전환 분기를 제거하고 WebM 영상, 2500ms 화면 교체, 2540ms 후반 재생, 최초 목적지 제목 애니메이션을 공통 적용한다.

> 2026-07-12: 모바일 지역 선택 시 2단계 탭과 금색 강조는 유지하되 `Tap again to enter` 보조 문구는 제거한다.

> 2026-07-12: 모바일 World Atlas 지역 진입을 2단계 탭으로 변경한다. 첫 탭은 해당 지역 hotspot을 금색으로 선택하고 `Tap again to enter` 안내를 노출하며, 같은 지역을 다시 탭해야 진입한다. 다른 지역을 누르면 선택만 이동하고 PC는 기존 단일 클릭 진입을 유지한다.

> 2026-07-12: 모바일에서는 구름 트랜지션을 완전히 비활성화한다. 840px 이하에서는 영상, 전환 제목, 지연 없이 목적지 화면을 즉시 교체하며 PC의 기존 WebM 구름 전환 타이밍은 유지한다.
